...
public:
    ...
    static int Read( unsigned char* );     // 読み込み
    
    static void AdvertiseStart();   // アドバタイズの開始
    static void AdvertiseStop();    // アドバタイズの停止
    static void SetPeriToCentData( std::string );
    ...
    static void SetReadData( unsigned char* );      // 読み込みデータ設定
...
private:
    ...
    static unsigned char sm_readData[512];              // 読み込みデータ
    static int sm_nCmdVal;
